...
export function resolveStudioSchemaValueMeta(
  ...
  const structure = resolveDataSchemaAuthoringStructure(schema, schemas, seen);
...
function collectNestedPaths(
  ...
  if (resolveStudioSchemaValueMeta(resolved, schemas, seen).type === 'array' && resolved.items) {
