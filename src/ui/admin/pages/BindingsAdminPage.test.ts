...
const literalAuthoringSource = await Bun.file(
  path.join(import.meta.dir, 'bindings/BindingLiteralAuthoringEditor.tsx'),
).text();
...
test('routes DataSchema-backed operation literals through the shared Authoring Engine', () => {
  expect(eventComposerSource).toContain('field.authoring ?');
  expect(eventComposerSource).toContain('<BindingLiteralAuthoringEditor');
  expect(literalAuthoringSource).toContain('deriveAuthoringModel');
  expect(literalAuthoringSource).toContain('<AuthoringEditor');
  expect(literalAuthoringSource).toContain('applyAuthoringMutation');
});
