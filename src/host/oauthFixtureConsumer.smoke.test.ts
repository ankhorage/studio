...
test(
  'generates the released Google and Apple OAuth fixture through the real host pipeline',
  async () => {
    ...
    try {
      ...
      const packageJson = JSON.parse(
        await readProjectFile(created.path, 'package.json'),
      ...
      expect(oauthRuntime).toContain("const OAUTH_TRANSPORT_ATTEMPT_KEY = 'ankh.auth.oauth.transport.v1';");
      ...
      const publicRuntimePaths = generatedFiles.filter(
        (file) =>
          file.startsWith('src/') || file === 'app.config.ts' || file === 'package.json',
      );
